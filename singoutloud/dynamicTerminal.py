...
def dynamic_terminal(playerObject: Player):
    try:
        while True:
            #Get input
            emit(SAVE_CURSOR, GOTO_INPUT, CLEAR_LINE)
            try:
                choice = input('Choice (type [H]elp for help): ')
                choice = choice.lower()
            except ValueError:
                continue
            finally:
            ...
            if choice in ("h", "help"):
                ...
                for index, song in enumerate(playerObject.songs):
                    print(f"{index + 1}. {song}")

            elif choice in ("p", "previous", "back"):
                playerObject.current_song_number -= 2
                playerObject.play()
